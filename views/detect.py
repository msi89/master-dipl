...
model_path = os.path.abspath(
    "faceai/trained_faces_model.clf")
...
def analyse_face(filename: str) -> str:
    print("GETTING HERE ...............")
    print("PATIENT >>>>>>>>>>>>>>>>>", diagnose_image(filename))
    status = diagnose_image(filename)[0]
    return status


def diagnose_image(filename: str):
...
def draw_landmarks(filename: str):

    detector = dlib.get_frontal_face_detector()

    predictor = dlib.shape_predictor(os.path.abspath(
        "faceai/shape_predictor_68_face_landmarks.dat"))
